...
<body>
    ...
    <section id="content">
        ...
        <main>
            <div class="container mx-auto px-4 py-6">
                <form method="POST" action="add_glass.php" class="max-w-lg mx-auto bg-white p-8 rounded-xl shadow-lg mt-10">
                    <h2 class="text-2xl font-semibold mb-6 text-center text-gray-800">Add New Glass</h2>
                    <div class="mb-5">
                        <label for="libelle" class="block text-sm font-medium text-gray-700 mb-1">Libelle</label>
                        <input type="text" name="libelle" id="libelle" class="block w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-400 focus:border-transparent" required>
                    </div>
                    <div class="mb-5">
                    ...
                    <div class="text-center">
                        <button type="submit" name="submit" class="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300">Add Glass</button>
                    </div>
                </form>
            </div>
        </main>
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO glass (libelle, type, type_glass, SPH, cylindre) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $libelle, $type, $type_glass, $SPH, $cylindre);

        if ($stmt->execute()) {
            echo "<script>alert('New glass added successfully.'); window.location.href='glasses.php';</script>";
        } else {
